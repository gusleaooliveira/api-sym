...
export class ExpensesService {
  ...
  async create(createExpensesDto: CreateExpenseDto) { 
    let Expenses = new this.ExpensesModel(createExpensesDto)
    return Expenses.save()
  }

  async findAll() {
    return this.ExpensesModel.find().populate('frequency').populate('type').populate('tag').exec()
  }

  async findOne(id: string) {
    return this.ExpensesModel.findById(id).populate('frequency').populate('type').populate('tag').exec();
  }
 
  async update(id: string, updateExpensesDto: UpdateExpenseDto) { 
    const Expenses = await this.ExpensesModel.findByIdAndUpdate(id, updateExpensesDto).exec()
